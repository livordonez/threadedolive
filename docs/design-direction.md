...
- Keep empty states primarily typographic when a final motif does not clearly improve the composition.
...
`components/make-story.tsx` owns the public make-post composition. Keep its hierarchy stable: title and main photograph first, the long-form story in a separate reading-width section, then details, gallery photography, and reflective notes.

The first uploaded image is the hero. When make details exist, the second image is the material swatch and later images form the gallery; without details, all secondary images remain in the gallery. The details section stays on the warm page background so only the small pinked swatch carries fabric texture. This prevents metadata from competing with the story or photography.

...
function Section({ section }: { section: PageSection }) {
  ...
  return section.links?.length ? <section>{section.heading ? <SectionHeading>{section.heading}</SectionHeading> : null}<ul className="mt-5 divide-y divide-olive-900/10 border-y border-olive-900/10">{section.links.map((link, index) => <li key={`${link.url}-${index}`}><a href={link.url} className="flex items-center justify-between gap-5 py-5"><span><span className="block font-serif text-2xl text-olive-900">{link.label}</span>{link.description ? <span className="mt-1 block text-sm text-charcoal-700">{link.description}</span> : null}</span><ExternalLinkIcon /></a></li>)}</ul></section> : null;
}

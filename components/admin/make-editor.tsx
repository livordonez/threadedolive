...
export function MakeEditor({ make }: { make: Make }) {
  return (
    <form action={saveMakeAction} className="space-y-8">
      ...
      <section className="admin-panel"><h2 className="admin-section-title">Photos</h2><p className="admin-help">The first photo is the main photo. The second becomes the fabric swatch on the project page, so use a close-up of the actual fabric when you have one. Use Earlier and Later to change the order.</p><div className="mt-5"><ImageUploader initialImages={make.images} /></div></section>
      ...
      <div className="sticky bottom-4 z-20 flex flex-wrap items-center gap-3 rounded-2xl border border-olive-900/10 bg-white/95 p-4 shadow-xl backdrop-blur"><button name="intent" value="draft" className="admin-button-secondary">Save Draft</button>{make.status === "published" ? <><button name="intent" value="publish" className="admin-button">Save Changes</button><button name="intent" value="unpublish" className="admin-button-secondary">Unpublish</button><a href={`/makes/${make.slug}?preview=1`} target="_blank" className="admin-button-secondary">Preview</a></> : <><button name="intent" value="publish" className="admin-button">Publish</button><a href={`/makes/${make.slug}?preview=1`} target="_blank" className="admin-button-secondary">Preview</a></>}<button formAction={deleteMakeAction} onClick={(event) => { if (!confirm("Delete this make permanently?")) event.preventDefault(); }} className="ml-auto text-sm font-semibold text-pimento-700">Delete</button></div>
    </form>
  );
}

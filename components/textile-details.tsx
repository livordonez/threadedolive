...
export function FabricSwatch({ image, label = "Material swatch" }: { image?: CmsImage; label?: string }) {
  return (
    <figure className="w-36">
      <PinkedEdge className="relative aspect-square bg-olive-200 shadow-sm">
        {image ? (
          <Image src={image.url} alt={image.alt || label} fill className="object-cover" sizes="144px" />
        ) : (
          <div className="mustard-textile h-full" />
        )}
      </PinkedEdge>
      <figcaption className="stitch-label mt-2 text-charcoal-700">{label}</figcaption>
    </figure>
  );
}

...
function Quick({ title, help, create, createLabel, href }: { title: string; help: string; create: AdminFormAction; createLabel: string; href: string }) { return <section className="admin-panel"><h2 className="admin-section-title">{title}</h2><p className="admin-help mt-2">{help}</p><div className="mt-5 flex flex-wrap items-start gap-2"><CreateActionForm action={create} label={createLabel} /><Link href={href} className="admin-button-secondary">View</Link></div></section>; }
